...
if (empty($sliders)) {
    $sliders = [
        ['image'=>'1.png','review_text'=>'Если вы еще не прыгали с парашютом, то многое потеряли. Это незабываемые впечатления, крутой подарок и красота нереальная!'],
    ];
}
if (empty($gratitudes)) {
    $gratitudes = [
        ['image'=>'11.jpg','caption'=>''],
        ['image'=>'22.jpg','caption'=>''],
        ['image'=>'33.jpeg','caption'=>''],
        ['image'=>'44.jpg','caption'=>''],
    ];
}
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const rawSliders = <?= $slidersJson ?>;
            const reviewsData = rawSliders.map(r => ({ file: r.image, text: r.review_text || '' }));
